...
import {
    ...
    answerCallbackQuery,
    applyTransactionEditsWithLLM,
    suggestOnboardingCategoriesWithLLM,
} from "@/lib/telegram";
...
function getMissingFields(transaction: PendingTransaction) {
    ...
    if (!Number.isFinite(transaction.amount) || Number(transaction.amount) <= 0)
        missing.push("amount");
...
function clarificationPrompt(
    ...
    if (field === "merchant")
        return "Nice, just to clarify, what merchant was this at?";
        ...
        keyboard: {
            inline_keyboard: [
                [
                    {
                        text: "Today",
                        callback_data: `pick_date:${sessionId}:today`,
                    },
                    {
                        text: "Yesterday",
                        callback_data: `pick_date:${sessionId}:yesterday`,
                    },
                ],
                [
                    {
                        text: "Custom (YYYY-MM-DD)",
                        callback_data: `pick_date:${sessionId}:custom`,
                    },
                ],
            ],
        },
        ...
        if (payload.message) {
            ...
            if (text.startsWith("/start")) {
                ...
                if (parts.length > 1) {
                    ...
                    // Verify code
                    const { data: linkingData, error: linkError } =
                        await supabase
                    ...
                    if (
                        linkError ||
                        !linkingData ||
                        new Date(linkingData.expires_at) < new Date()
                    ) {
                        await sendMessage(
                            chatId,
                        ...
                        const { data: linkedProfile, error: updateError } =
                            await supabase
                                .from("profiles")
                                .upsert(
                                    {
                        ...
                        if (
                            updateError ||
                            !linkedProfile ||
                            linkedProfile.telegram_chat_id !== chatId
                        ) {
                            ...
                            await sendMessage(
                                chatId,
                                '✅ Account successfully linked! You can now send me your transactions like "Spent 15.50 on lunch" or "Starbucks $5".',
                            );
            ...
            if (!profile) {
                await sendMessage(
                    chatId,
                    "⚠️ Your account is not linked. Please use /link in the app to get your connection code.",
                );
                return NextResponse.json({ ok: true });
            }
            ...
            const { data: currentSession } = await supabase
                ...
                .select(
                    "id, state, clarification_field, pending_transactions, session_meta",
                )
                .eq("chat_id", chatId)
            ...
            if (currentSession?.state === "onboarding_profile") {
                if (text.trim().toLowerCase() === "skip") {
                    ...
                    await sendMessage(
                        chatId,
                        "Skipped category bootstrap. You can start logging transactions now.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                try {
                    const suggested =
                        await suggestOnboardingCategoriesWithLLM(text);
                    ...
                    if (
                        ...
                        await sendMessage(
                            chatId,
                            '⏱️ Setup timed out. Try again with a shorter profile, or type "skip".',
                        );
                    } else {
                        await sendMessage(
                            chatId,
                            '❌ Could not generate categories. Type "skip" to continue without bootstrap.',
                        );
                    }
                }
                return NextResponse.json({ ok: true });
            ...
            if (currentSession?.state === "awaiting_clarification") {
                const pendingList = Array.isArray(
                    currentSession.pending_transactions,
                )
                    ? (currentSession.pending_transactions as PendingTransaction[])
                    : [];
                const active = pendingList[0] || {};
                const field = (currentSession.clarification_field ||
                    "merchant") as "merchant" | "amount" | "category" | "date";
                const updated = fillDraftField(active, field, text);

                if (!updated) {
                    await sendMessage(
                        chatId,
                        `⚠️ I still need your ${field}. ${clarificationPrompt(field)}`,
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (missing.length > 0) {
                    ...
                    if (missing[0] === "category") {
                        ...
                        await supabase
                            ...
                            .update({
                                session_meta: {
                                    category_options: prompt.options,
                                },
                                updated_at: new Date().toISOString(),
                            })
                            .eq("id", currentSession.id);
                ...
                const keyboard = {
                    inline_keyboard: [
                        [
                            {
                                text: "✅ Log It",
                                callback_data: `log_session:${currentSession.id}`,
                            },
                            {
                                text: "❌ Cancel",
                                callback_data: `cancel_session:${currentSession.id}`,
                            },
                        ],
                    ],
                };
            ...
            if (
                currentSession?.state === "awaiting_confirmation" &&
                !text.startsWith("/")
            ) {
                const pendingList = Array.isArray(
                    currentSession.pending_transactions,
                    ...
                    ? (currentSession.pending_transactions as PendingTransaction[])
                    : [];
                const currentTransactions = pendingList.filter(
                ...
                if (currentTransactions.length > 0) {
                    ...
                    try {
                            ...
                            const keyboard = {
                                inline_keyboard: [
                                    [
                                        {
                                            text:
                                                editResult.transactions.length >
                                                1
                                                    ? "✅ Log All"
                                                    : "✅ Log It",
                                            callback_data: `log_session:${currentSession.id}`,
                        ...
                        if (
                            ...
                            await sendMessage(
                                chatId,
                                '⏱️ Edit timed out. Try a shorter instruction like "set all dates to 2026-02-07".',
                            );
                        } else {
                            await sendMessage(
                                chatId,
                                "❌ Could not apply that edit. Please try again.",
                            );
                        }
                        return NextResponse.json({ ok: true });
                    }
            ...
            try {
                const parseResult = await parseTransactionWithLLM(
                    text,
                    categoryNames,
                    messageDate,
                );
                ...
                if (parseResult.kind === "clarify") {
                    ...
                    if (
                        missing.length === 0 &&
                        isCompleteTransaction(parseResult.draft)
                    ) {
                            ...
                            const keyboard = {
                                inline_keyboard: [
                                    [
                                        {
                                            text: "✅ Log It",
                                            callback_data: `log_session:${session.id}`,
                                        },
                                        {
                                            text: "❌ Cancel",
                                            callback_data: `cancel_session:${session.id}`,
                                        },
                                    ],
                                ],
                            };
                ...
                if (!session?.id) {
                    await sendMessage(
                        chatId,
                        "❌ Failed to create confirmation session. Please try again.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (error instanceof Error && error.message === "LLM_TIMEOUT") {
                    await sendMessage(
                        chatId,
                        '⏱️ Parsing timed out. Please try a shorter message like "Spent 12.50 on lunch".',
                    );
                } else {
                    await sendMessage(
                        chatId,
                        "❌ Sorry, I couldn't parse that transaction. Try being more specific, e.g., 'Spent $10 at Subway'.",
                    );
                }
            }
        }
        ...
        if (payload.callback_query) {
            ...
            if (data.startsWith("onboarding:")) {
                ...
                if (mode === "skip") {
                    ...
                    await sendMessage(
                        chatId,
                        "No problem. You can start logging transactions now.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (mode === "defaults") {
                    ...
                    await sendMessage(
                        chatId,
                        `✅ Added ${applied.length} default categories. Start logging whenever you’re ready.`,
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                const options =
                    (session?.session_meta as SessionMeta | null)
                        ?.category_options || [];
                ...
                if (!session || !picked) {
                    ...
                    await sendMessage(
                        chatId,
                        "❌ Category options expired. Please type the category.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (!session) {
                    ...
                    await sendMessage(
                        chatId,
                        "❌ Session expired. Please send the transaction again.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (mode === "custom") {
                    ...
                    await sendMessage(
                        chatId,
                        "Please type the date as YYYY-MM-DD.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                if (transactions.length === 0) {
                    ...
                    await sendMessage(
                        chatId,
                        "❌ No valid transactions found in this confirmation. Please send them again.",
                    );
                    return NextResponse.json({ ok: true });
                }
                ...
                for (const transaction of transactions) {
                    let categoryId =
                        categoryCache.get(transaction.category) ?? null;
                ...
                if (error) {
                    ...
                    await sendMessage(
                        chatId,
                        `✅ Successfully logged <b>${transactions.length}</b> transaction${transactions.length > 1 ? "s" : ""}.`,
                    );
                }
            }
        }
